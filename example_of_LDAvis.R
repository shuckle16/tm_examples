...
t <- LDA(dtm[-which(rowSums(as.matrix(dtm)) == 0),],3)

dtmlda <- dtm[-which(rowSums(as.matrix(dtm))==0),]

crplda <- crp[-which(rowSums(as.matrix(dtm))==0)]


j <- topicmodels_json_ldavis(t,crplda,dtmlda)
